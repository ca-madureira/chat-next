...
const UsersPopup = (props) => {
  ...
  const handleSelect = async (user) => {
    try {
      ...
      const res = await getDoc(doc(db, 'chats', combinedId)); //retorno de determinado chat que esta na coleção chats.

      if (!res.exists()) {
        //se determinado chat não existir, ele adiciona o chat
        await setDoc(doc(db, 'chats', combinedId), { messages: [] });

        const currentUserChatRef = await getDoc(
          //retorna a lista de chats que o usuario logado possui
          doc(db, 'userChats', currentUser.uid)
        );
        //retorna a lista de chats que esse usuario possui
  ...
  return (
    <PopupWrapper {...props} title="Adicionar amigo">
      <Search />
